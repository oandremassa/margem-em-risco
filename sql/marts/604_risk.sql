...
PRINT N'Criando camada materializada: score de risco';
GO

CREATE OR ALTER VIEW mart.calc_contract_risk_score
AS
WITH observed AS
(
    SELECT
...
SELECT TOP (0) *
INTO mart.contract_risk_score_data
FROM mart.calc_contract_risk_score;
GO
...
CREATE UNIQUE CLUSTERED INDEX CIX_contract_risk_score_data
ON mart.contract_risk_score_data (date_key, contract_key);
GO

CREATE OR ALTER VIEW mart.vw_contract_risk_score
AS
SELECT *
FROM mart.contract_risk_score_data;
GO

IF (SELECT COUNT(*) FROM mart.contract_risk_score_data) <> 10
    THROW 51513, 'O score de risco deveria conter 10 contratos.', 1;
IF EXISTS
(
    SELECT 1 FROM mart.contract_risk_score_data
    WHERE contract_risk_score NOT BETWEEN 0 AND 100
)
    THROW 51514, 'Existe score de risco fora da faixa de 0 a 100.', 1;
GO

PRINT N'Camada concluida: score de risco';
GO
