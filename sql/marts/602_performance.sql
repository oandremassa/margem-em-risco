...
PRINT N'Criando camada materializada: desempenho mensal';
GO

CREATE OR ALTER VIEW mart.calc_contract_monthly_performance
AS
...
SELECT TOP (0) *
INTO mart.contract_monthly_performance_data
FROM mart.calc_contract_monthly_performance;
GO
...
CREATE UNIQUE CLUSTERED INDEX CIX_contract_monthly_performance_data
ON mart.contract_monthly_performance_data (date_key, contract_key);
GO

CREATE OR ALTER VIEW mart.vw_contract_monthly_performance
AS
SELECT *
FROM mart.contract_monthly_performance_data;
GO

IF (SELECT COUNT(*) FROM mart.contract_monthly_performance_data) <> 10
    THROW 51511, 'Desempenho mensal deveria conter 10 contratos.', 1;
GO

PRINT N'Camada concluida: desempenho mensal';
GO
