...
PRINT N'Criando camada materializada: fila mensal de acoes';
GO

CREATE OR ALTER VIEW mart.calc_action_priority_queue_monthly
AS
...
SELECT TOP (0) *
INTO mart.action_priority_queue_monthly_data
FROM mart.calc_action_priority_queue_monthly;
GO
...
CREATE UNIQUE CLUSTERED INDEX CIX_action_priority_queue_monthly_data
ON mart.action_priority_queue_monthly_data (date_key, contract_key);
GO

CREATE OR ALTER VIEW mart.vw_action_priority_queue_monthly
AS
SELECT *
FROM mart.action_priority_queue_monthly_data;
GO

IF (SELECT COUNT(*) FROM mart.action_priority_queue_monthly_data) <> 10
    THROW 51516, 'A fila mensal deveria conter 10 contratos.', 1;
GO

PRINT N'Camada concluida: fila mensal de acoes';
GO
